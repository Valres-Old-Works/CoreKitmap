...
class IPBanCommand extends Command
{
    public function __construct() {
        parent::__construct("ip-ban", "Ban IP un joueur", "usage : /ip-ban <player> <time> <?reason>");
        $this->setPermission("ip-ban.command");
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void {
        ...
        foreach($IPs as $IP){
            if(!$sanctionManager->isIPBanned($IP)){
                $sanctionManager->addIPBan(new IPBan($playerName, $IP, $reason, $time, $sender->getName()));
            }
        }
