...
class ReducemoneyCommand extends Command
{
    public function __construct() {
        parent::__construct("reduce-money", "Réduit de la money à un joueur", "usage : /reduce-money <player> <amount>");
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void {
        ...
        $moneyManager->reduceMoney($target, $amount);
        $sender->sendMessage(str_replace(
            ["{amount}", "{target}"],
            [$amount, $target],
            $config->get("player-reduce-money-message")
        ));
        ...
        if($targetPlayer instanceof Player){
            $targetPlayer->sendMessage(str_replace(
                ["{amount}", "{player}"],
                [$amount, $target],
                $config->get("target-reduce-money-message")
            ));
        }
    }
}