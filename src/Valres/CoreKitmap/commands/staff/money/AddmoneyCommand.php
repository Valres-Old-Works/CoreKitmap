...
class AddmoneyCommand extends Command
{
    public function __construct() {
        parent::__construct("add-money", "Ajoute de la money à un joueur", "usage : /add-money <player> <amount>");
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void {
        ...
        $sender->sendMessage(str_replace(
            ["{amount}", "{target}"],
            [$amount, $target],
            $config->get("player-add-money-message")
        ));
        ...
        if($targetPlayer instanceof Player){
            $targetPlayer->sendMessage(str_replace(
                ["{amount}", "{player}"],
                [$amount, $target],
                $config->get("target-add-money-message")
            ));
        }
    }
}