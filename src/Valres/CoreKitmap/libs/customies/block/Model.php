...
final class Model {
	 ...
	 */
	public function __construct(array $materials, int $collision = 1, ?string $geometry = null, ?Vector3 $origin = null, ?Vector3 $size = null) {
		$this->materials = $materials;
		$this->geometry = $geometry;
		$this->origin = $origin ?? Vector3::zero();
		$this->size = $size ?? Vector3::zero();
        $this->collision = $collision ?? 1;
	}
	...
	public function toNBT(): array {
		...
		if($this->geometry === null) {
			...
			$material["minecraft:collision_box"] = CompoundTag::create()
				->setByte("enabled", $this->collision)
